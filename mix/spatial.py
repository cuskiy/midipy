...
_XFEED_LP = None
_CEILING_SOS = None
_HRTF_CACHE = BoundedCache(64)

_ER_TAPS = [
    (0.011, 0.25, -48), (0.020, 0.25, 50), (0.031, 0.18, 35),
    (0.041, 0.18, -40), (0.054, 0.10, -8),
]
_ER_TAPS_IMMERSIVE = [
    (0.009, 0.22, -45), (0.015, 0.22, 52), (0.023, 0.16, -30),
    (0.029, 0.20, 38), (0.036, 0.15, -42), (0.044, 0.12, 15),
    (0.052, 0.08, -10), (0.065, 0.06, 25),
]
...
def _compute_hrtf_irs(az_deg, length, pinna):
    """Compute HRTF impulse responses for a given azimuth."""
    ...
    return li_hi, ri_hi, li_lo, ri_lo
...
def apply_hrtf(mono, az_deg, length=64, pinna=False):
    ...
    _ensure_xover()
    cache_key = (round(az_deg, 1), pinna, length)
    if cache_key not in _HRTF_CACHE:
        _HRTF_CACHE[cache_key] = _compute_hrtf_irs(az_deg, length, pinna)
    li_hi, ri_hi, li_lo, ri_lo = _HRTF_CACHE[cache_key]
